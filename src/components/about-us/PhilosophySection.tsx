 ...
 interface PhilosophyItem {
   ...
   const philosophyItems: PhilosophyItem[] = [
    ...
    },
    {
        ...
        bgColor : "bg-green-300 text-black-500"
...

    {
        ...
        description : "#TRUST",
        hashtag : "#TRUST",
        bgColor : "bg-pink-300  text-white-300"
    }
  ];
    ...
    <section id="philosophy" className="py-20 bg-pink-50">
      <div className="text-center mb-12 px-4">
        <h2 className="text-4xl font-bold text-black">
          Our Philosophy in <span className="bg-gradient-to-r from-purple-500 via-pink-500 to-yellow-500 text-transparent bg-clip-text">Action</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-4 lg:px-16">
        {philosophyItems.map((item) => (
          <div
            key={item.id}
            className={`rounded-xl p-6 min-h-[160px] flex flex-col justify-between ${item.bgColor}`}
          >
            <div className="text-sm font-semibold">0{item.id}</div>
            <div className="mt-2 flex-1 font-bold">{item.title}</div>
